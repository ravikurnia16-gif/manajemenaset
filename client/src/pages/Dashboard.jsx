...
import { Box, DollarSign, AlertTriangle, TrendingDown, Loader2, Clock, Wrench, Sparkles, ChevronRight } from 'lucide-react';
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {canFilterUnit && filterUnit === 'all' && data?.unitStats?.length > 0 && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden animate-in zoom-in-95 duration-300">
                    ...
                    <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl shadow-xl overflow-hidden p-6 flex flex-col gap-4 text-white">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                <Sparkles className="text-blue-400" size={20} />
                                <h3 className="font-bold text-sm uppercase tracking-wider">Predictive Alerts</h3>
                            </div>
                            <span className="px-2 py-0.5 bg-blue-500/20 text-blue-300 rounded text-[10px] font-bold border border-blue-500/30 tracking-widest">AI ENGINE</span>
                        </div>
                        
                        <div className="flex-1 space-y-4">
                            {!data?.dueSoonAssets || data.dueSoonAssets.length === 0 ? (
                                <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-3 opacity-50">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px]">Belum ada jadwal pemeliharaan mendesak.</p>
                                    </div>
                                </div>
                            ) : (
                                data.dueSoonAssets.map((asset, i) => (
                                    <div key={i} className="bg-white/5 hover:bg-white/10 rounded-xl p-3 border border-white/5 transition-all group cursor-pointer">
                                        <div className="flex justify-between items-start mb-2">
                                            <span className="text-[10px] font-black text-blue-400 uppercase tracking-tighter">Est. {new Date(asset.nextMaintenanceEst).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}</span>
                                            <Wrench size={12} className="text-slate-500 group-hover:text-blue-400 transition-colors" />
                                        </div>
                                        <div className="font-bold text-xs truncate uppercase tracking-tight">{asset.name}</div>
                        ...
                        <div className="pt-2 border-t border-white/5">
                            <p className="text-[9px] text-slate-500 italic text-center">
                                *Prediksi berdasarkan riwayat pemeliharaan & umur manfaat aset.
                            </p>
                        </div>
                    </div>
                </div>
