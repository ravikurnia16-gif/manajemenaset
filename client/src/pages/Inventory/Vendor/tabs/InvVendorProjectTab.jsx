        ...
        <div className="space-y-6">
            ...
            {filteredProjects.length === 0 ? (
                <div className="bg-white rounded-2xl border border-slate-100 p-8 text-center text-slate-400">
                    Tidak ada proyek pengadaan dalam filter ini.
                </div>
            ) : (
                            ...
                            <div key={project.id} className="bg-white rounded-2xl border border-slate-200 shadow-2xs overflow-hidden transition-all hover:border-blue-200">
                                <div className="p-4 border-b border-slate-100 flex flex-wrap justify-between items-start gap-3 bg-slate-50/70">
                                    <div className="flex-1 min-w-[280px]">
                                        ...
                                        <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500 mb-2">
                                            <span className="flex items-center gap-1">
                                                ...
                                                Pemohon: <b className="text-slate-700">{project.requestedByName || 'Bagian Sarana'}</b>
                                            </span>
                                            {project.targetDate && (
                                                <span className="flex items-center gap-1">
                                            ...
                                            <span>Tipe: <b className="text-slate-600">{project.projectType || project.type || 'SELEKSI'}</b></span>
                                        </div>
                                    ...
                                    <div className="flex flex-wrap gap-2 items-center">
                                        {/* 1. Tombol Lembar Persetujuan (ACC Kabid) */}
                                        <button 
                                        ...
                                        {/* 2. Tombol Surat Pesanan (PO) - Enabled hanya setelah ACC */}
                                        {isApproved ? (
                                            <button 
                                            ...
                                            <button 
                                                disabled 
                                                title="Surat Pesanan baru dapat diterbitkan setelah disetujui oleh Kepala Bidang Sarana"
                                        ...
                                        {/* 3. Tombol Penerimaan Barang (Cek Fisik & BAST) */}
                                        {isApproved && project.status !== 'SELESAI' && (
                                            ...
                                            >
                                                <CheckCircle size={14} /> Terima Barang & BAST
                                            </button>
                                        )}

                                        {/* 4. Tombol Lihat BAST Resmi jika sudah terbit */}
                                        {project.bastNumber && (
                                            <button 
                                        ...
                                        {isApproved ? (
                                            ...
                                            <button 
                                                disabled 
                                                title="Vendor seleksi dapat ditambahkan setelah disetujui Kepala Bidang Sarana"
                                ...
                                {/* Daftar Vendor Peserta Seleksi */}
                                <div className="p-4">
                                    <h4 className="text-xs font-bold text-slate-600 uppercase tracking-wider mb-3">
                                        Daftar Vendor Peserta Seleksi / Rekanan
                                    </h4>
                                    {(!project.selections && !project.vendorSelections || (project.selections?.length === 0 && project.vendorSelections?.length === 0)) ? (
                                        <p className="text-xs text-slate-400 italic">
                                            {isPendingApproval ? 'Proyek sedang menunggu persetujuan Kepala Bidang Sarana sebelum pemilihan vendor.' : 'Belum ada vendor yang dimasukkan ke tahap seleksi proyek ini.'}
                                        </p>
                                    ) : (
                                        <div className="overflow-x-auto">
                                            <table className="w-full text-xs">
                                                ...
                                                <tbody className="divide-y divide-slate-50">
                                                    {(project.selections || project.vendorSelections || []).map(sel => (
